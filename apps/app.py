...
with st.sidebar:
    st.markdown('<div class="sidebar-title">📈 株兄さん</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="sidebar-note">学習・診断・比較・メモを1つにまとめたアプリだぜ。</div>',
        unsafe_allow_html=True,
    )
    st.divider()
    st.caption("ページ一覧")
...
pg = st.navigation([page for _, page in pages])
pg.run()
